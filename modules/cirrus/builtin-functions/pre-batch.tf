...
# ------------------------------------------------------------------------------
data "aws_iam_policy_document" "cirrus_pre_batch_lambda_assume_role" {
  ...
  statement {
    ...
    condition {
      ...
      values   = ["arn:aws:lambda:${local.current_region}:${local.current_account}:function:${var.resource_prefix}-pre-batch"]
    }
    condition {
      test     = "StringEquals"
...
resource "aws_iam_role" "cirrus_pre_batch_lambda" {
  name_prefix        = "${var.resource_prefix}-pre-batch-"
  description        = "Lambda execution role for Cirrus builtin Task 'pre-batch'"
  assume_role_policy = data.aws_iam_policy_document.cirrus_pre_batch_lambda_assume_role.json
}
# ==============================================================================
...
# ------------------------------------------------------------------------------
resource "aws_iam_role_policy_attachment" "cirrus_pre_batch_lambda_vpc_access" {
  role       = aws_iam_role.cirrus_pre_batch_lambda.name
...
# ------------------------------------------------------------------------------
data "aws_iam_policy_document" "cirrus_pre_batch_lambda_role_main" {
...
resource "aws_iam_role_policy" "cirrus_pre_batch_lambda_role_main" {
  name_prefix = "${var.resource_prefix}-pre-batch-policy-"
  role        = aws_iam_role.cirrus_pre_batch_lambda.name
  policy      = data.aws_iam_policy_document.cirrus_pre_batch_lambda_role_main.json
}
# ==============================================================================
...
resource "aws_cloudwatch_metric_alarm" "cirrus_pre_batch_lambda_errors_warning_alarm" {
  ...
  alarm_name                = "WARNING: ${var.resource_prefix}-pre-batch Lambda Errors Warning Alarm"
  alarm_description         = "${var.resource_prefix}-pre-batch Lambda Errors Warning Alarm"
  statistic                 = "Sum"
  metric_name               = "Errors"
...
resource "aws_cloudwatch_metric_alarm" "cirrus_pre_batch_lambda_errors_critical_alarm" {
  ...
  alarm_name                = "CRITICAL: ${var.resource_prefix}-pre-batch Lambda Errors Critical Alarm"
  alarm_description         = "${var.resource_prefix}-pre-batch Lambda Errors Critical Alarm"
  statistic                 = "Sum"
  metric_name               = "Errors"
  comparison_operator       = "GreaterThanOrEqualToThreshold"
  threshold                 = 100
